...
for iternum in range(n_experimental_iters):
    ...
    if len(observed_idx) == 0:
        ...
        radius_estimated = onp.exp(
            fitted_params[0] + 0.5 * jnp.exp(fitted_params[3]) ** 2
        )
        slope_estimated = fitted_params[1]
        preds = model.predict(X, radius_estimated, slope_estimated)
...
radius_estimated = onp.exp(fitted_params[0] + 0.5 * jnp.exp(fitted_params[2]) ** 2)
slope_estimated = fitted_params[1]
preds = model.predict(X, radius_estimated, slope_estimated)
...
radius_estimated = onp.exp(
    fitted_params_naive[0] + 0.5 * jnp.exp(fitted_params_naive[2]) ** 2
)
slope_estimated = fitted_params_naive[1]
preds_naive = model.predict(X, radius_estimated, slope_estimated)
import ipdb
